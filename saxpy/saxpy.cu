...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    // CS149: allocate device memory buffers on the GPU using cudaMalloc.
    //    
    gpuErrchk(cudaMalloc(&device_x, N * sizeof(float)));
    gpuErrchk(cudaMalloc(&device_y, N * sizeof(float)));
    gpuErrchk(cudaMalloc(&device_result, N * sizeof(float)));
    ...
    //
    // CS149: copy input arrays to the GPU using cudaMemcpy
    //
    ...
    //
    gpuErrchk(cudaMemcpy(resultarray, device_result, N * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    gpuErrchk(cudaFree(device_result));
}
